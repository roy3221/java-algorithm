 ...
 *  @author Kevin Wayne
 */
public class FordFulkersonEdgeChange {
    private static final double FLOATING_POINT_EPSILON = 1E-11;

    private boolean[] marked;     // marked[v] = true iff s->v path in residual graph
    private FlowEdge[] edgeTo;    // edgeTo[v] = last edge on shortest residual s->v path
    private double value;         // current value of max flow
    private FlowEdge change;
    private int cf=0,ct=0,cc=0;
    private int t=0,s=0;
    private int[] bottle;
    ...
    public FordFulkersonEdgeChange(FlowNetwork G, int s, int t,int cf, int ct, int cc) {
        validate(s, G.V());
        validate(t, G.V());
    ...
    // Chu Rong
    public void update(FlowNetwork G){
    	double maxflow=0;
    	double oldCapacity=0;
    	double tokens=0;
    	value=0;
    	FlowEdge e1 = null;
    	for (FlowEdge e :G.adj(cf)) {
    		if(e.to()==ct){
    			e.addResidualFlowTo(cf, e.flow()-cc);
    			oldCapacity=e.capacity();
    			e.capacity=cc;
    			tokens=e.flow()-cc;
    		}
    	}
    	for (FlowEdge e :G.adj(ct)) {
    		if(e.to()==t){
    			e.addResidualFlowTo(ct, e.flow()-cc);
    		}
    	}
    	while(tokens>0){
    		for (FlowEdge e :G.adj(cf)) {
    		...
    		if(e1.flow()>oldCapacity){
            	e1.addResidualFlowTo(e1.other(cf), e1.flow()-cc);
        	}
        	else{
        		e1.addResidualFlowTo(e1.other(cf), e1.flow());
        		tokens=tokens-e1.flow();
        	}
    	}
    	 for (int v = 0; v < G.V(); v++) {
             for (FlowEdge e : G.adj(v)) {
                 if (inCut(v) && e.flow()==e.capacity()){
                	 value+=e.flow();
                 }
             }
    	 }
    ...
    public static void main(String[] args) {
        ...
        In in = new In(args[0]);
        int s = Integer.parseInt(args[1]), t = Integer.parseInt(args[2]), cf=Integer.parseInt(args[3]), ct=Integer.parseInt(args[4]),cc=Integer.parseInt(args[5]);
        FlowNetwork G = new FlowNetwork(in);
        FlowEdge change =new FlowEdge(cf, ct, cc);
       
        StdOut.println(G);
        // compute maximum flow and minimum cut
        FordFulkersonEdgeChange maxflow = new FordFulkersonEdgeChange(G, s, t,cf,ct,cc);
        ...
        StdOut.println();
        //Chu Rong: 
        StdOut.println("edges corresponding to the minimum cut: ");
        for (int v = 0; v < G.V(); v++) {
            for (FlowEdge e : G.adj(v)) {
                if (maxflow.inCut(v) && e.flow()==e.capacity())
                    StdOut.println("   " + e);
            }
        }
        //Chu Rong end.
        ...
        StdOut.println();
        StdOut.print("The edge that is changed:");
